...
file_path = '/egr/research-dselab/renjie3/renjie/NeurIPS24_concept_removal/concept_removal/data/i2p_part_llama2_13b.jsonl'
llama213b_results = []
with open(file_path, 'r') as file:
    for line in file:
        dictionary = json.loads(line.strip())
...
for gpt_example, llama_example in zip(gpt_results, llama213b_results):
    ...
    llama_vio_label = llama_line.split('violence: ')[1].split(';')[0].strip()
    ...
    llama_sex_label = llama_line.split('obscene: ')[1].split(';')[0].strip()
    if gpt_vio_label != llama_vio_label:
        sex_n_eq += 1
    ...
    gpt_specific_label = gpt_line.split('specific: ')[1].strip().split(';')[0].strip().replace('.', '')
    llama_specific_label = llama_line.split('specific: ')[1].strip().split(';')[0].strip().replace('.]', '')

    # if gpt_specific_label != 'False' or llama_specific_label != 'False':
    #     print(gpt_specific_label)
    #     print(llama_specific_label)
